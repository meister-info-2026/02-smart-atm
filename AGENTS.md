...
9. 조작이 1분간 없으면 ATM은 대기 화면으로 돌아간다(`IDLE_RESET_SECONDS`). 앞사람이
   남긴 화면을 뒤에 온 사람이 물려받지 않게 하려는 것이다. **단, 제한이 걸려 있는
   동안에는 세지 않는다** — 차단·상담원 확인 중, 그리고 설정 오류로 서버에 물어보지
   못하는 중에는 카운트다운이 아예 뜨지 않는다
10. **제한을 푸는 것은 시간도 버튼도 아니고 사람이다.** 화면의 "처음으로"는 잠긴
   기계를 열지 못하고(거절된다), 상담원이 `RELEASED`로 확인하거나 은행 직원이
   "직원 확인 후 해제"(`POST /staff-release`)를 눌러야 다시 열린다. 잠긴 기계는
   다른 QR로도 열리지 않는다 — 안전한 문자로 QR을 새로 만들어 오면 풀린다면
   그것도 해제 버튼이다. 직원 해제도 **서버의 판정을 지우지 않으므로** 같은 QR을
   다시 비추면 즉시 다시 막힌다
